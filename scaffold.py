...
def %(lower_model)s_list(request, template='%(lower_model)s/%(lower_model)s_list.html'):
    d = {}
    d['form'] = %(model)sForm()
...
DETAILS_VIEW = """
from %(app)s.forms import %(model)sForm
def %(lower_model)s_details(request, id, template='%(lower_model)s/%(lower_model)s_details.html'):
    d = {}
...
APP_TEMPLATE_CONTENT ="""
    ...
    {%% block application_navbar %%}
        ...
        {%% if request.path == '/%(app)s/' %%}
            {{ block.super }}
            {%% include "%(app)s/%(app)s_landing_page_navbar.html" %%}
        {%% else %%}
            {{ block.super }}
            {%% include "%(app)s/%(app)s_landing_page_navbar.html" %%}
            {#        {%% include "%(app)s/%(app)s_landing_page_navbar.html" %%}#}
            {%% include "%(app)s/%(app)s_navbar.html" %%}
            {%% include "%(app)s/projectManager_navbar2.html" %%}
            {%% include "%(app)s/left_sidebar.html" %%}
        {%% endif %%}
...
class Scaffold(object):
    ...
    def create_app(self):
        ...
        if not path.exists('{0}{1}'.format(self.SCAFFOLD_APPS_DIR, self.app)):
            ...
            self._info('There is no such application: {}\n Creating application ...'.format(self.app))

            system('python manage.py startapp {0}'.format(self.app))
            # system('mv {0} {1}'.format(self.app, self.SCAFFOLD_APPS_DIR))
    ...
    def create_templates(self):
        ...
        if path.exists('{0}{1}/templates/{2}/{2}_list.html'.format(self.SCAFFOLD_APPS_DIR, self.app,
                                                               self.model.lower())):
            self._info('exists\t{0}{1}/templates/{2}/{2}_list.html'.format(
                self.SCAFFOLD_APPS_DIR, self.app, self.model.lower()), 1)
        else:
            with open("{0}{1}/templates/{2}/{2}_list.html".format(self.SCAFFOLD_APPS_DIR, self.app,
            ...
            self._info('create\t{0}{1}/templates/{2}/{2}_list.html'.format(
                self.SCAFFOLD_APPS_DIR, self.app, self.model.lower()), 1)

        # ====== DETAILS ========================
        # Check if model_detail.html exists

        if path.exists('{0}{1}/templates/{2}/{2}_detail.html'.format(
                self.SCAFFOLD_APPS_DIR, self.app, self.model.lower())):
            self._info('exists\t{0}{1}/templates/{2}/{1}_detail.html'.format(
                self.SCAFFOLD_APPS_DIR, self.app, self.model.lower()), 1)
        else:
            with open("{0}{1}/templates/{2}/{2}_detail.html".format(
            ...
            self._info('create\t{0}{1}/templates/{2}/{2}_detail.html'.format(
                self.SCAFFOLD_APPS_DIR, self.app, self.model.lower()), 1)


    def create_app_template(self):

        self._info("    APP template   ")
        self._info("===========")


        # Check if application template file exists

        if path.exists('{0}{1}/templates/{1}_app_template.html'.format(self.SCAFFOLD_APPS_DIR, self.app)):
            self._info('exists\t{0}{1}/templates/{1}_app_template.html'.format(self.SCAFFOLD_APPS_DIR, self.app), 1)
        else:
            with open("{0}{1}/templates/{1}_app_template.html".format(self.SCAFFOLD_APPS_DIR, self.app,
                                                                  self.model.lower()), 'w') as fp:
                fp.write(APP_TEMPLATE_CONTENT % {
                    'model': self.model.lower(),
                    'title': self.model.lower(),
                    'app': self.app,
                })
            self._info('create\t{0}{1}/templates/{1}_app_template.html'.format(
                self.SCAFFOLD_APPS_DIR, self.app, self.model.lower()), 1)
